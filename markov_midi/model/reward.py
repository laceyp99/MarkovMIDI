...
- Rating criteria (overall, melodic, harmonic, rhythmic, cohesion)
...
class Rating:
    ...
    Attributes:
        ...
        melodic: Melody line quality
        harmonic: Chord progression quality
        rhythmic: Timing and groove quality
    ...
    def __post_init__(self) -> None:
        ...
        for name in ["overall", "melodic", "harmonic", "rhythmic", "cohesion"]:
    ...
    def to_chord_reward(self) -> float:
        """
        Calculate chord model reward from ratings.

        Combines harmonic and rhythmic ratings, centered around 3.
        """
        # Center ratings around 0 (3 stars = neutral)
        harmonic_delta = self.harmonic - 3
        rhythmic_delta = self.rhythmic - 3
        overall_delta = self.overall - 3
        cohesion_delta = self.cohesion - 3

        # Weight harmonic most heavily for chord model
        return (
            harmonic_delta * 1.5
            + rhythmic_delta * 0.5
            + overall_delta * 0.5
    ...
    def to_melody_reward(self) -> float:
        """
        Calculate melody model reward from ratings.

        Combines melodic and rhythmic ratings, centered around 3.
        """
        ...
        # Weight melodic most heavily for melody model
        return (
            melodic_delta * 1.5
            + rhythmic_delta * 0.5
            + overall_delta * 0.5
...
class RewardManager:
    ...
    def apply_rating(
        ...
        # Apply to chord model
        if chord_reward != 0.0 and record.chord_transitions:
            generator.chord_model.apply_reward(
                record.chord_transitions,
                chord_reward,
                self.sensitivity_multiplier,
            )

        # Apply to melody model
        if melody_reward != 0.0 and record.melody_transitions:
            generator.melody_model.apply_reward(
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "average_rhythmic": sum(r.rhythmic for r in ratings) / rated,
