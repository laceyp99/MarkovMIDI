...
class ChordModel:
    ...
    def generate(
        ...
        Args:
            ...
            start_degree: Starting chord degree (default 1 = tonic)
            end_on_tonic: If True, ensure progression ends on I chord
        ...
        current_time = 0
        prev_degree = start_degree
        prev_prev_degree = start_degree
        ...
        while current_time < total_16ths:
            ...
            next_degree, chord_trans = self.chord_chain.sample(chord_context, rng=rng)
    ...
    def apply_reward(
        ...
        for context, next_state in transitions:
            # Determine which chain based on context/state values
            if next_state in CHORD_DEGREES:
                # Check if it looks like a chord transition
            ...
            elif next_state in CHORD_RHYTHM_DURATIONS:
                ctx = cast(tuple[int, int] | tuple[int], context)
